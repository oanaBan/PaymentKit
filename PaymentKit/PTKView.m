...
// Oana change
- (PTKAddressZip *)cardZipCode {
    return [PTKAddressZip addressZipWithString:self.cardZipCodeField.text];
}
...
- (BOOL)cardZipCodeShouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)replacementString
{
    ...
    resultString = [PTKTextField textByRemovingUselessSpacesFromString:resultString];
    PTKAddressZip *zipCode = [PTKAddressZip addressZipWithString:resultString];
...
- (PTKTextField *)firstInvalidField;
{
    ...
    else if (([PTKTextField textByRemovingUselessSpacesFromString:self.cardZipCodeField.text].length > 0) && ![[PTKAddressZip addressZipWithString:self.cardZipCodeField.text] isValid]) // Oana change
